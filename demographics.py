...
COHORT_PATH = "/oscar/data/class/biol1595_2595/students/mspancho/ie_cohort.csv"
RESULTS_DIR = "/oscar/data/class/biol1595_2595/students/mspancho/"
...
summary.to_csv(f"{RESULTS_DIR}/demographics_summary.csv", index=False)

N = len(df)

# ── Raw Numbers: Age Bins ─────────────────────────────────────────────────────
age_bins = np.histogram_bin_edges(df["anchor_age"], bins=6)
age_counts, _ = np.histogram(df["anchor_age"], bins=age_bins)
...
for label, count in race_counts.items():
    pct = count / N * 100
    print(f"{label:<30} {count:<10} {pct:.1f}%")
# ── Raw Numbers: Gender / Sex ─────────────────────────────────────────────
print("\n=== SEX ===")
if "sex" in df.columns:
    gender_counts = df["sex"].value_counts()
elif "gender" in df.columns:
    gender_counts = df["gender"].map({"M": "Male", "F": "Female"}).fillna("Unknown").value_counts()
else:
