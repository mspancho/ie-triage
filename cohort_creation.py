...
diagnoses = pd.read_csv(
    os.path.join(MIMIC_HOSP, "diagnoses_icd.csv"),
...
edstays = pd.read_csv(
    os.path.join(MIMIC_ED, "edstays.csv"),
...
ed_triage = ed_triage.merge(patients[["subject_id", "gender", "anchor_age"]], on="subject_id", how="left", suffixes=("", "_patients"))
...
# Keep only the demographic columns we need, duplicates with _admissions suffix
...
# Stratum A: high mimicry conditions with ICD codes that resemble IE at triage
mimic_codes = [
    "A40", "A41", "038",  # sepsis
    "J18", "J15", "4801", "481", "482", "484", "486", "487", "5168",  # pneumonia
    "G00", "G03", "3209", "322",  # meningitis (CNS) infection
    "R50",  # fever of unknown origin
    "M00", "M86", "0389", "730",  # septic arthritis + osteomyelitis
    "I63", "434",  # stroke
    "I50", "428",  # heart failure
    "I30", "I40", "420", "42292"  # acute pericarditis + myocarditis
]

# Stratum B: moderate mimicry by risk factors or presenting symptoms
...
negatives["has_risk_keyword"] = negatives["chiefcomplaint_text"].str.contains(risk_keyword_pattern, regex=True)
...
if len(sampled_negatives) < target_controls:
    ...
    if fill > 0:
        sampled_negatives = pd.concat([sampled_negatives, remaining.sample(n=fill, random_state=42)], axis=0)
...
cohort_out = cohort[["subject_id", "stay_id", "hadm_id", "intime", "outtime", "ie_label", *demographic_cols, *feature_cols]].copy()

# Normalize race categories to balance expressivity with statistical power.
...
# Validate and clean data to ensure plausible values
...
# Vital signs: filter to plausible ranges
vital_ranges = {
    'temperature': (95, 105),  # Fahrenheit
    'heartrate': (40, 200),    # bpm
    'resprate': (8, 40),       # breaths/min
    'o2sat': (70, 100),        # %
    'sbp': (70, 250),          # mmHg
    'dbp': (40, 150),          # mmHg
}
...
for col, (min_val, max_val) in vital_ranges.items():
    ...
    cohort_out = cohort_out[(cohort_out[col].isna()) | ((cohort_out[col] >= min_val) & (cohort_out[col] <= max_val))]
...
# Dates: ensure intime < outtime and valid
...
cohort_out.to_csv(os.path.join(OUTPUT_DIR, "proj1_ie_triage_cohort.csv"), index=False)

summary = pd.DataFrame({
    "metric": [
        "ie_positive_count",
        "target_negative_controls",
        "selected_negative_controls",
...
summary.to_csv(os.path.join(OUTPUT_DIR, "proj1_ie_cohort_summary.csv"), index=False)

print(f"Wrote cohort to {os.path.join(OUTPUT_DIR, 'proj1_ie_triage_cohort.csv')}")
print(f"Wrote cohort summary to {os.path.join(OUTPUT_DIR, 'proj1_ie_cohort_summary.csv')}")
