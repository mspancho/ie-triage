"""
Feature engineering pipeline with multiple chief complaint representation options.

Produces 4 separate feature matrices:
  1. Categorical chief complaints + demographics + vitals
  2. PCA-reduced BERT embeddings + other features
  3. Hybrid (categorical + PCA embeddings + other features)
  4. No chief complaint features (demographics + vitals only)
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Feature engineering with multiple CC representation options.",
    )
    parser.add_argument("--cohort-csv", type=str, required=True,
                        help="Path to cohort CSV (output of cohort_creation.py)")
    parser.add_argument("--output-dir", type=str, required=True,
                        help="Directory for output .npz files (data/npz)")
...
def main():
    ...
    # ── 5. Build 4 feature matrices ─────────────────────────────────────────
    print("\nAssembling feature matrices …")

    X_opt1 = np.concatenate([cont_arr, esi_arr, cat_arr, cc_cat_arr], axis=1)
    fn_opt1 = cont_feature_names + esi_feature_names + cat_feature_names + cc_cat_feature_names
    print(f"Option 1 (Categorical): X shape: {X_opt1.shape}")

    X_opt2 = np.concatenate([cont_arr, esi_arr, cat_arr, cc_embeddings_pca], axis=1)
    fn_opt2 = cont_feature_names + esi_feature_names + cat_feature_names + cc_pca_feature_names
    print(f"Option 2 (PCA Embeddings): X shape: {X_opt2.shape}")

    X_opt3 = np.concatenate([cont_arr, esi_arr, cat_arr, cc_cat_arr, cc_embeddings_pca], axis=1)
    fn_opt3 = cont_feature_names + esi_feature_names + cat_feature_names + cc_cat_feature_names + cc_pca_feature_names
    print(f"Option 3 (Hybrid): X shape: {X_opt3.shape}")

    X_opt4 = np.concatenate([cont_arr, esi_arr, cat_arr], axis=1)
    fn_opt4 = cont_feature_names + esi_feature_names + cat_feature_names
    print(f"Option 4 (No Chief Complaint): X shape: {X_opt4.shape}")

    # ── 6. Save ─────────────────────────────────────────────────────────────
    option_labels = ["categorical", "pca_embeddings", "hybrid", "no_cc"]
    option_data = [(X_opt1, fn_opt1), (X_opt2, fn_opt2), (X_opt3, fn_opt3), (X_opt4, fn_opt4)]
    for opt_num, (X, feature_names) in enumerate(option_data, start=1):
        out_path = os.path.join(args.output_dir, f"features_option{opt_num}_{option_labels[opt_num - 1]}.npz")
